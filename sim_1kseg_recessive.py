...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep 24 12:38:06 2019
...
def update_par_file(region_name, temp_par, new_par,insert,dominance,adm_amount,adm_time,sel_time): #dominance is a string = "additive" or "recessive" or "partial"
    ...
    for line_counter, line in enumerate(oldfile):
        ...
        if line_counter==readLine:
            #print(fields)
            fields[2] = 'readFile("/u/home/x/xinjunzh/scripts_simulation_treeseq/1ksegments/sim_seq_info_'+str(region_name)+'.txt");'
        elif line_counter == outtempLine:
            fields[0] = 'sim.outputFull("/u/scratch/x/xinjunzh/slim_sim-sel_newtest/temp/'+str(region_name)+'_" '
        elif line_counter == insertLine:
            fields[1] = str(insert)+");"
        elif line_counter == readtempLine:
            fields[0] = 'sim.readFromPopulationFile("/u/scratch/x/xinjunzh/slim_sim-sel_newtest/temp/'+str(region_name)+'_" '          
        elif line_counter == treeLine:
            fields[0] = 'sim.treeSeqOutput("/u/scratch/x/xinjunzh/slim_sim-sel_newtest/'+str(region_name)+'_" '          
...
if __name__ == "__main__":
    os.chdir("/u/home/x/xinjunzh/scripts_simulation_treeseq/")
    ...
    DIR_stat = "/u/scratch/x/xinjunzh/simstats_allvar-sel_newtest/recessive/"
    os.mkdir(DIR_stat+thisgene)
    
    num_rep = 1000
    
    slim_template = "/u/scratch/x/xinjunzh/slim_sim-sel/"+"h_NEW_sim_cluster_recessive_adaptive.slim"
    slim_thisgene = "/u/scratch/x/xinjunzh/slim_sim-sel_newtest/"+thisgene+"-h_NEW_sim_cluster_recessive_adaptive_newtest.slim"
    
    #window_start,window_end=find_mut_site ("segments/"+thisfile)
    #insert = int((int(window_end)+int(window_start))/2)
    
    
    #update_par_file(thisgene, slim_template, slim_thisgene,insert,"recessive")
    ...
    while rep < num_rep:
        ...
        try:
            ...
            update_par_file(thisgene, slim_template, slim_thisgene,insert,"recessive",adm_amount,adm_time,sel_time)
#1. run slim, get tree file
        #os.system('/u/home/x/xinjunzh/slim_build/slim sim_cluster.slim')
        
        #update slim file
            slim_output = "/u/scratch/x/xinjunzh/slim_sim-sel_newtest/"+thisgene +"_allvar_"+ "-slim_output_recessive_adaptive.txt"
...
#2. from tree to vcf
            filename = "/u/scratch/x/xinjunzh/slim_sim-sel_newtest/"+thisgene +"_allvar_"+ 'test_recessive_adaptive'
            scalingfactor = 10
...
#from vcf to stats 
    ...
    #pos_list.index(these)
            ...
            outputname = thisgene+"output_recessive_adaptive_"
 ...
 #       if (rep+1)%1000 ==0:
 #       	outdir = DIR_stat+thisgene+"/"
 #       	command = '''
 #       	cd {0}
 #       	awk '
 #       		FNR==1 && NR!=1 { while (/^rep/) getline; }
 #       		1 {print}
 #       	' *.txt > ../{1}_all_recessive_{2}.txt
        	
 #       	'''.format(outdir,thisgene,str(rep))
 #       	os.system(command)
