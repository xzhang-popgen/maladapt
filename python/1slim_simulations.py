...
def update_par_file(region_name, temp_par, new_par,insert,dominance,adm_amount,adm_time,sel_time): #dominance is a string = "additive" or "recessive" or "partial"
    ...
    for line_counter, line in enumerate(oldfile):
        ...
        if line_counter==readLine:
            #print(fields)
            fields[2] = 'readFile("/u/scratch/x/xinjunzh/slim/1ksegments/sim_seq_info_'+str(region_name)+'.txt");'
        elif line_counter == outtempLine:
            fields[0] = 'sim.outputFull("/u/scratch/x/xinjunzh/slim/'+dominance+'/temp/'+str(region_name)+'_" '
            fields[1] = '+"misspec-original_"+'
        elif line_counter == insertLine:
            fields[1] = str(insert)+");"
        elif line_counter == readtempLine:
            fields[0] = 'sim.readFromPopulationFile("/u/scratch/x/xinjunzh/slim/'+dominance+'/temp/'+str(region_name)+'_" '
            fields[1] = '+"misspec-original_"+'          
        elif line_counter == treeLine:
            fields[0] = 'sim.treeSeqOutput("/u/scratch/x/xinjunzh/slim/'+dominance+'/tree/'+str(region_name)+'_" '
            fields[1] = '+"misspec-original_"+'          
...
if __name__ == "__main__":
    os.chdir("/u/scratch/x/xinjunzh/slim/")
    ...
    DIR_stat = "/u/scratch/x/xinjunzh/slim/"+dominance+"/stat/"
    os.mkdir(DIR_stat+thisgene)
    
    num_rep = 1000 #for every segment, sim 1k replicates 
    
    slim_template = "/u/scratch/x/xinjunzh/slim/"+dominance+"_adaptive.slim"
    slim_thisgene = "/u/scratch/x/xinjunzh/slim/"+dominance+"/"+thisgene+"-misspec-original_"+dominance+"_adaptive.slim"
    
    
    adm_range = [0.01,0.02,0.05,0.1] #adm amount range
    ...
    while rep < num_rep:
        ...
        try:
            ...
            insert = int((int(window_end)+int(window_start))/2)   #find an exon in the middle of the segment to insert mutation
            adm_time = random.choice(range(8697,8747))   #adm time range
            sel_time = random.choice(range(0,61))   #neutrality time, upper boundary = adm time
            update_par_file(thisgene, slim_template, slim_thisgene,insert,dominance,adm_amount,adm_time,sel_time)
#1. run slim, get tree file
        
        #update slim file
            slim_output = "/u/scratch/x/xinjunzh/slim/"+dominance+"/"+thisgene +"_misspec-original_"+ "-slim_output_"+dominance+"_adaptive.txt"
...
#2. from tree to vcf
            filename = "/u/scratch/x/xinjunzh/slim/"+dominance+"/tree/"+thisgene +"_misspec-original_"+ 'test_'+dominance+'_adaptive'
            scalingfactor = 10
            mu = 1.5e-8 * scalingfactor
            archaic_sample_time = 152.0

    #number of individuals from [afr, asn]
            sampsize=[108,99] #Neanderthal is 1

    #get exon definitions
            exonfilepath = segs_path+thisfile 
...
#from vcf to stats 
            ...
            os.system("cp "+vcfpath+" /u/scratch/x/xinjunzh/slim/"+dominance+"/VCF/")
    ...
    #each = []
            ...
            outputname = thisgene+"_misspec-original_"+dominance+"_adaptive_"
