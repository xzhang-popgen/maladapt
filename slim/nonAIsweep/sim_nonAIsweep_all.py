...
parser.add_argument('-d', '--dominance', action="store", dest="dominance_id",
                        help="which dominance model, default: 1; range 1-3",
                        default=1, type=int)

args = parser.parse_args()

batch = str(args.sim_id) #now sim id is going to refer to a 
whichgene = int(args.gene_id-1) 
whichdom = int(args.dominance_id-1) 

dominance_list = ["recessive","partial","additive"]

dominance = dominance_list[whichdom]
...
def update_par_file(region_name, temp_par, new_par,dominance,insert,adm_amount,adm_time): #dominance is a string = "additive" or "recessive" or "partial"
    if (dominance == "recessive"):
        ...
        adm_amountLine = 87
        adm_timeLine = 84
        adm_relatedLine1 = 103
        adm_relatedLine2 = 121
    ...
    for line_counter, line in enumerate(oldfile):
        ...
        if line_counter==readLine:
            #print(fields)
            fields[2] = 'readFile("'+DIR_master+'1ksegments/sim_seq_info_'+str(region_name)+'.txt");'
        elif line_counter == outtempLine:
            fields[0] = 'sim.outputFull("'+DIR_master+'nonAIsweep/'+dominance+'/temp/'+str(region_name)+'_" '
        elif line_counter == insertLine:
            fields[1] = str(insert)+");"
        elif line_counter == readtempLine:
            fields[0] = 'sim.readFromPopulationFile("'+DIR_master+'nonAIsweep/'+dominance+'/temp/'+str(region_name)+'_" '          
        elif line_counter == treeLine:
            fields[0] = 'sim.treeSeqOutput("'+DIR_master+'nonAIsweep/'+dominance+'/tree/'+str(region_name)+'_" '          
        elif line_counter == adm_timeLine:
            fields[0] = str(adm_time) 
        elif line_counter == adm_amountLine:
            fields[2] = str(adm_amount) +","
            fields[3] = str(1-adm_amount)+"));"
        elif line_counter == adm_relatedLine1:
            fields[0] = str(adm_time+1)+":"
        elif line_counter == adm_relatedLine2:
            fields[0] = str(adm_time+1) 
...
if __name__ == "__main__":
    DIR_master = "/u/scratch/x/xinjunzh/slim_nonAIsweep/"
    ...
    DIR_stat = "nonAIsweep/"+dominance+"/test_stat/"
    
    thisgene = genes[whichgene-1]
    thisfile = allfiles[whichgene-1]

    
    num_rep = 100 #100 replicates per segment
    
    slim_template = "nonAIsweep/"+"h_NEW_sim_cluster_"+ dominance+"_nonAIsweep.slim"
    slim_thisgene = "nonAIsweep/"+dominance+"/"+thisgene+"-h_NEW_sim_cluster_"+dominance+"_nonAIsweep.slim"
    ...
    while rep < num_rep:
        try:
            ...
            update_par_file(thisgene, slim_template, slim_thisgene,dominance,insert,adm_amount,adm_time)
#1. run slim, get tree file
        
        #update slim file
            slim_output = "nonAIsweep/"+dominance+"/"+thisgene +"_allvar_"+ "-slim_output_"+dominance+"_sweep.txt"
            os.system('%s %s > %s' %(Path_slim,slim_thisgene,slim_output))
            print("slim done")
            m2_s,m2_in_p4 = getSelectedAlleleSelcoeff(slim_output)
            m2_s = float(m2_s)

#2. from tree to vcf
            filename = "nonAIsweep/"+dominance+"/tree/"+thisgene +"_allvar_"+ 'test_'+dominance+'_sweep'
            scalingfactor = 10
            mu = 1.5e-8 * scalingfactor
            archaic_sample_time = 152.0

    #number of individuals from [afr, asn]
            sampsize=[100, 100] #Neanderthal is 1
...
#from vcf to stats 
    ...
    #len(windows)
            ...
            allpos_digitized = np.digitize(pos_list, allpos_bin)


    #these = [pos for pos in pos_list if (pos <= windows[w][1] and pos >= windows[w][0])]
    #pos_list.index(these)

            outputname = dominance+"-"+thisgene+"output_recessive_sweep_"
        
        #DIR_stat = "/u/scratch/x/xinjunzh/simstats/recessive/"
        #os.mkdir(DIR_stat+thisgene)
            ...
            os.remove(slim_thisgene)
            temp_out = 'nonAIsweep/'+dominance+'/temp/'+thisgene+"_allvar_temp_"+dominance+".txt"
            os.remove(temp_out)
            tree_files = 'nonAIsweep/'+dominance+'/tree/'+thisgene+"_allvar_"+dominance+"_sweep*"
