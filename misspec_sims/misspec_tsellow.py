...
if __name__ == "__main__":
    ...
    num_rep = 100 #repeat random choice 100 times
    
    slim_template = "/u/scratch/x/xinjunzh/slim_nonAIsweep/misspec/"+"misspec-segment_"+dominance+"_adaptive.slim"
    slim_thisgene = "/u/scratch/x/xinjunzh/slim_nonAIsweep/misspec/"+dominance+"/"+thisgene+"-misspec-tsellow_"+dominance+"_adaptive_newtest.slim"
    ...
    while rep < num_rep:
        print(rep)
        try:
            adm_amount = random.choice(adm_range) #randomize adm amount
            window_start,window_end=find_mut_site ("1ksegments/"+thisfile) #find an exon to insert mutation
            insert = int((int(window_end)+int(window_start))/2)   
            adm_time = random.choice([8746])   #fix adm time 
            sel_time = random.choice([62,72,82,92,102,112])   #set up sel time new boundary, up to 15000 years 
...
#1. run slim, get tree file
        #update slim file
            slim_output = "/u/scratch/x/xinjunzh/slim_nonAIsweep/misspec/"+dominance+"/"+thisgene +"_misspec-tsellow_"+ "-slim_output_"+dominance+"_adaptive.txt"
...
#2. from tree to vcf
            filename = "/u/scratch/x/xinjunzh/slim_nonAIsweep/misspec/"+dominance+"/test_tree/"+thisgene +"_misspec-tsellow_"+ 'test_'+dominance+'_adaptive'
            scalingfactor = 10
...
#from vcf to stats 
    ...
    #pos_list.index(these)
            ...
            outputname = thisgene+"_misspec-tsellow_"+dominance+"_adaptive_"
