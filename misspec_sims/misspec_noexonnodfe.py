...
def find_mut_site_noexonnodfe (segfile):
    segs = open(segfile)
    starts = []
    ends = []
    total = 0
...
def update_par_noexonnodfe_file(region_name, temp_par, new_par,insert,dominance,adm_amount,adm_time,sel_time): #dominance is a string = "additive" or "recessive" or "partial"
...
if __name__ == "__main__":
    ...
    num_rep = 100    
    slim_template = "/u/scratch/x/xinjunzh/slim_nonAIsweep/misspec/"+"misspec-noexonnodfe_"+dominance+"_adaptive.slim"
    slim_thisgene = "/u/scratch/x/xinjunzh/slim_nonAIsweep/misspec/"+dominance+"/"+thisgene+"-misspec-noexonnodfe_"+dominance+"_adaptive_newtest.slim"    
    adm_range = [0.01,0.02,0.05,0.1]
    rep=0
    while rep < num_rep:
        print(rep)
        try:
            #noexonnodfe_amount = noexonnodfe_range[rep]
            ...
            update_par_noexonnodfe_file(thisgene, slim_template, slim_thisgene,insert,dominance,adm_amount,adm_time,sel_time)
#1. run slim, get tree file
            slim_output = "/u/scratch/x/xinjunzh/slim_nonAIsweep/misspec/"+dominance+"/"+thisgene +"_misspec-noexonnodfe_"+ "-slim_output_"+dominance+"_adaptive.txt"
...
#2. from tree to vcf
            #filename = "/Users/xinjunzhang/Desktop/chr3region_misspec-noexonnodfe_test_additive_adaptive"
            filename = "/u/scratch/x/xinjunzh/slim_nonAIsweep/misspec/"+dominance+"/test_tree/"+thisgene +"_misspec-noexonnodfe_"+ 'test_'+dominance+'_adaptive'
            scalingfactor = 10
...
#from vcf to stats 
    ...
    #len(windows)
            ...
            outputname = thisgene+"_misspec-noexonnodfe_"+dominance+"_adaptive_"
