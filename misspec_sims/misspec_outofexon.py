...
def find_mut_site_outofexon (segfile):
    segs = open(segfile)
    starts = []
    ends = []
    total = 0
...
def update_par_outofexon_file(region_name, temp_par, new_par,insert,dominance,adm_amount,adm_time,sel_time): #dominance is a string = "additive" or "recessive" or "partial"
...
if __name__ == "__main__":
    ...
    num_rep = 100    
    slim_template = "/u/scratch/x/xinjunzh/slim_nonAIsweep/misspec/"+"misspec-outofexon_"+dominance+"_adaptive.slim"
    slim_thisgene = "/u/scratch/x/xinjunzh/slim_nonAIsweep/misspec/"+dominance+"/"+thisgene+"-misspec-outofexon_"+dominance+"_adaptive_newtest.slim"    
    adm_range = [0.01,0.02,0.05,0.1]
    rep=0
    while rep < num_rep:
        print(rep)
        try:
            #outofexon_amount = outofexon_range[rep]
            adm_amount = random.choice(adm_range)
            window_start,window_end=find_mut_site ("1ksegments/"+thisfile)#find_mut_site_outofexon ("1ksegments/"+thisfile)
            insert = int((int(window_end)+int(window_start))/2)   
            adm_time = random.choice(range(8697,8747))   
            sel_time = random.choice(range(0,61))   
            update_par_outofexon_file(thisgene, slim_template, slim_thisgene,insert,dominance,adm_amount,adm_time,sel_time)
#1. run slim, get tree file
            slim_output = "/u/scratch/x/xinjunzh/slim_nonAIsweep/misspec/"+dominance+"/"+thisgene +"_misspec-outofexon_"+ "-slim_output_"+dominance+"_adaptive.txt"
...
#2. from tree to vcf
            filename = "/u/scratch/x/xinjunzh/slim_nonAIsweep/misspec/"+dominance+"/test_tree/"+thisgene +"_misspec-outofexon_"+ 'test_'+dominance+'_adaptive'
            scalingfactor = 10
...
#from vcf to stats 
    ...
    #len(windows)
            ...
            outputname = thisgene+"_misspec-outofexon_"+dominance+"_adaptive_"
