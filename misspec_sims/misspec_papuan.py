...
######################################################################
def getSelectedAlleleSelcoeff_papuan(file_name): # returns true if m2 was fixed in p1 before admixture
    ...
    for line in infile:
        if line[0:5]=='#OUT:':
            fields = line.split()
            selcoeff = fields[7]
            if (int(fields[1]) == 34693) & (fields[3] == "p8"):
            	m2_in_p4 = 1
            #break
    infile.close()
...
def ancestry_p_varies(ts,pop,nsize,duration): #pop=source pop
    ...
    today = [x.id for x in ts.nodes() if ((x.population == 8) and (x.time == 0))] #assuming p8 is recipient
...
def find_mut_site_papuan (segfile):
    segs = open(segfile)
    starts = []
    ends = []
    total = 0
    for line_counter,line in enumerate (segs):
        if (line[0:3]=="cnc" or line[0:3]=="neu"):      
            fields = line.split()
            if (int(fields[1]) >= 2200000) & (int(fields[2]) <=2800000):
                starts.append(fields[1])
                ends.append(fields[2])
...
def update_par_papuan_file(region_name, temp_par, new_par,insert,dominance): #dominance is a string = "additive" or "recessive" or "partial"
...
if __name__ == "__main__":
    ...
    num_rep = 100    
    slim_template = "/u/scratch/x/xinjunzh/slim_nonAIsweep/misspec/"+"misspec-papuan_"+dominance+"_adaptive.slim"
    slim_thisgene = "/u/scratch/x/xinjunzh/slim_nonAIsweep/misspec/"+dominance+"/"+thisgene+"-misspec-papuan_"+dominance+"_adaptive_newtest.slim"    
    rep=0
    while rep < num_rep:
        print(rep)
        try:
            #papuan_amount = papuan_range[rep]
            window_start,window_end=find_mut_site ("1ksegments/"+thisfile)#find_mut_site_papuan ("1ksegments/"+thisfile)
            insert = int((int(window_end)+int(window_start))/2)   
            update_par_papuan_file(thisgene, slim_template, slim_thisgene,insert,dominance)
#1. run slim, get tree file
            slim_output = "/u/scratch/x/xinjunzh/slim_nonAIsweep/misspec/"+dominance+"/"+thisgene +"_misspec-papuan_"+ "-slim_output_"+dominance+"_adaptive.txt"
            os.system('/u/home/x/xinjunzh/slim_build/slim %s > %s' %(slim_thisgene,slim_output))
            print("slim done")
            m2_s,m2_in_p4 = getSelectedAlleleSelcoeff_papuan(slim_output)
            m2_s = float(m2_s)

#2. from tree to vcf
            #filename = "/Users/xinjunzhang/Desktop/chr3region_misspec-papuan_test_additive_adaptive"
            filename = "/u/scratch/x/xinjunzh/slim_nonAIsweep/misspec/"+dominance+"/test_tree/"+thisgene +"_misspec-papuan_"+ 'test_'+dominance+'_adaptive'
            scalingfactor = 10
            mu = 1.5e-8 * scalingfactor
            archaic_sample_time = 138.0
            adm_amount=0.04
            sel_time=0
...
#from vcf to stats 
            ...
            meanp1_D2 = calc_p1ancestry(treepath,4,883,34693-adm_time-1) #treepath, admpop, popsize,t_sinceadm
            meanp1_D1 = calc_p1ancestry(treepath,5,883,34693-adm_time-1) #treepath, admpop, popsize,t_sinceadm
            meanp1 = meanp1_D2+meanp1_D1
    ...
    #len(windows)
            ...
            outputname = thisgene+"_misspec-papuan_"+dominance+"_adaptive_"
